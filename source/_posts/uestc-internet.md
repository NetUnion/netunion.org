...
5-6 期公寓及新修建的博士公寓的移动宽带和电信宽带，使用学校提供的网页认证方式接入，需要将学校统一身份认证账号与宽带账号绑定。操作步骤如下：

#### 连接路由器

> 连接路由器并不属于网管会的工作内容，如有需要请自行解决。该操作原理并不复杂，可通过各类搜索引擎搜索相关教程。
...
2. 使用网线连接路由器的 WAN 口和墙面网口，或者直接连接电脑和墙面网口。一般来说 WAN 口就是最独特的那一个端口(一般在最边上)，有特殊的颜色或者标识，其他端口是 LAN 口。部分路由器不需要区分 WAN 和 LAN，只需要连接任意一个端口即可。
    ...
    ![WAN口和LAN口](/assets/20240901161545.jpg)

    找到WAN口后，用网线将WAN口与网络插座连接，如下。

    ![WAN口连网络插座](/assets/20240901162039.jpg)
...
4. 如果您的电脑有网口，可以直接使用网线连接电脑和路由器的 LAN 口，如果没有网口，可以连接到路由器的无线网络。路由器的标签上写有默认的 WiFi 名称和密码（如果有密码），如下图：
    ...
    ![路由器的标签](/assets/20240901162150.jpg)
...
> 如果打开 `10.253.0.235` 后无法正常登录自服务（可能是深澜系统后台暂无您的账号信息），可先输入学号和密码，随便选择一家运营商登录一次（此举为了同步账号信息），弹出消息（并非报错），耐心等几分钟，之后再登录自服务即可正常使用。

1. 在宿舍区，将电脑连接到上一步设置好的路由器或者直接连接到墙面网口。
2. 网络登陆页面可能会自动打开，如果没有打开，请手动访问 [http://10.253.0.235](http://10.253.0.235)。
...
6. 输入您的宽带账号和密码，点击 **“绑定”**。
    ...
    - 电信用户的账号就是手机号，密码一般是 `12345678`，如果不对，试试手机号后 8 位，或者联系电信客服重置密码。
...
1. 打开浏览器，访问任意网站，会自动跳转到登录页面。或者手动访问 [http://10.253.0.235](http://10.253.0.235)。
